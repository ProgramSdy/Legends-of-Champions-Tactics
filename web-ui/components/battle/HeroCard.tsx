...
export function HeroCard({ hero, active = false }: { hero: CombatantState; active?: boolean }) {
  return (
    <article className={`hero-card ${hero.sideId} ${active ? "active" : ""} ${hero.alive ? "" : "defeated"}`} aria-label={`${hero.displayName}, ${hero.specialization}${active ? ", active hero" : ""}`}>
      <AssetImage request={{ kind: "portrait", key: hero.id, name: hero.displayName, className: hero.specialization }} className="portrait" />
      <div className="hero-data">
        <div className="hero-title"><div><h3>{hero.displayName}</h3><p>{hero.specialization}{hero.isSummon ? " · Summon" : ""}</p></div><AssetImage request={{ kind: "class", key: hero.specialization, name: hero.specialization }} className="class-icon" /></div>
